                                    ...
                                    @foreach ($categories as $category)
                                        <li class="list-item">
                                            <span class="menu-link py-1">
                                                ...
                                                <label for="chk-category-1">{{ $category->name }}</label>
                                            </span>
                ...
                <div class="d-flex justify-content-between mb-4 pb-md-2">
                    ...
                    <div
                        ...
                        <select class="shop-acs__select form-select w-auto border-0 py-0 order-1 order-md-0"
                            style="margin-right:20px;" id="pagesize" name="pagesize">
                                    ...
                                    @if (Cart::instance('wishlist')->content()->where('id', $product->id)->count() > 0)
                                        <form action="{{ route('wishlist.item.remove', ['rowId' => Cart::instance('wishlist')->content()->where('id', $product->id)->first()->rowId]) }}"
                                            ...
                                            <button type="submit"
                                                ...
                                                style="color: orange" title="Remove from Wishlist">
    ...
    <script>
        $(function() {
            ...
            $("input[name='brands[]']").on("change", function() {
                ...
                $("#hdnBrands").val(brands.join(",")); // تحديث الحقل المخفي بالقيم المحددة
                console.log("Selected brands: ", brands.join(",")); // لمراقبة القيم المختارة
                $("#frmfilter").submit(); // إرسال الفورم تلقائيًا عند التغيير
            });
            ...
            $("input[name='categories']").on("change", function() {
                ...
                $("#hdnCategories").val(categories.join(",")); // تحديث الحقل المخفي بالقيم المحددة
                console.log("Selected categories: ", categories.join(",")); // لمراقبة القيم المختارة
                $("#frmfilter").submit(); // إرسال الفورم تلقائيًا عند التغيير
            });
